...
async function studentRoutes(fastify) {
    ...
    fastify.post('/:studentId/courses/:courseId', { preHandler: [authenticate] }, async (request, reply) => {
        if (request.user.role !== 'ADMIN')
            return reply.status(403).send({ error: 'No autorizado' });
        ...
        if (student.role !== 'STUDENT')
            return reply.status(400).send({ error: 'El usuario no es un estudiante' });
    ...
    fastify.delete('/:studentId/courses/:courseId', { preHandler: [authenticate] }, async (request, reply) => {
        if (request.user.role !== 'ADMIN')
            return reply.status(403).send({ error: 'No autorizado' });
        const studentId = Number(request.params.studentId);
        const courseId = Number(request.params.courseId);
