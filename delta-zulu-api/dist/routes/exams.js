...
async function examRoutes(fastify) {
    ...
    fastify.get('/:id', { preHandler: [authenticate] }, async (request, reply) => {
        ...
        if (!exam)
            return reply.status(404).send({ error: 'Examen no encontrado' });
        return exam;
    });
    // ADMIN: Create Exam
    ...
    fastify.delete('/:id', { preHandler: [authenticate] }, async (request, reply) => {
        ...
        if (isNaN(examId))
            return reply.status(400).send({ error: 'ID de examen inválido' });
        try {
    ...
    fastify.post('/:id/attempt', { preHandler: [authenticate] }, async (request, reply) => {
        const { id } = request.params;
